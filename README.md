...
- **单标的 HTTP**：`GET /api/backtest/ma-cross?code=sh.000001&fast=5&slow=20&limit=500`  
  ...
  - 返回总收益、买入持有、**超额**、最大回撤、夏普 / **Sortino**、**Calmar**、年化与波动、**多头持仓段**统计（段数、**段胜率（按段首前一日累计权益加权）**、**段均收益 %**）、**underlying_beta** / **underlying_alpha_ann_pct**（日策略收益对日标的收盘收益的 OLS，无风险利率 0，α×252 年化到百分点）、翻转次数、权益曲线采样点。
