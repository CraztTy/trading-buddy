"use strict";
/**
 * 方式 B + Playwright UI：终端一已 `npm run e2e:preview` 后，在本脚本中设好
 * PLAYWRIGHT_BASE_URL，preflight 探测，再打开 `playwright test --ui`。
 *
 * 用法：`npm run test:e2e:ui:connected` 或 `npm run test:e2e:ui:connected -- e2e/backtest-panel.spec.js`
 */
