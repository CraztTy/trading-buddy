"use strict";
/**
 * 方式 B + 固定 PLAYWRIGHT_CHANNEL（终端一已 `npm run e2e:preview`）。
 *
 * 用法：
 *   npm run test:e2e:connected:chromium
 *   npm run test:e2e:connected:chrome -- e2e/main-nav-smoke.spec.js
 */
...
if (ch !== "chromium" && ch !== "chrome") {
  process.stderr.write(
    "[e2e] usage: node ./e2e/run-e2e-connected-with-channel.cjs chromium|chrome [-- playwright-args...]\n"
  );
  process.exit(1);
}
